package io.github.orionlibs.orion.core.file_system.file;
...
import io.github.orionlibs.orion.core.file_system.file.tasks.AppendToFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.ConvertFileToStringListTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.ConvertFileToStringTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.ConvertURLToFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.CreateFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.DeleteFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.DownloadFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.GetFileExtensionTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.MoveFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.ReadFirstLineFromFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.SaveFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.SaveStringToFileTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.SaveTemporaryFileToServerTask;
import io.github.orionlibs.orion.core.file_system.file.tasks.UploadFileTask;
import io.github.orionlibs.orion.core.file_system.stream.FileSystemStreamsService;
...
public class FileService extends OrionService
{
    ...
    {
        List<String> lines = convertFileToStringList("D:/downloads/subjects.txt");
        String content = "";
        Path pairsFile = new File("D:/downloads/subject-pairs.txt").toPath();
    
        for(int i = 0; i < lines.size() - 1; i++)
        {
    
            if(lines.get(i).trim().length() > 2)
            {
    
                for(int j = i + 1; j < lines.size(); j++)
                {
    
                    if(lines.get(j).trim().length() > 2)
                    {
                        content = lines.get(i).trim() + "----" + lines.get(j).trim();
                        ...
                        try
                        {
                            ...
                            Files.write(pairsFile, content.getBytes(), openOptions);
                        }
                        catch(IOException e)
                        {
                            e.printStackTrace();
                ...
                System.out.println("" + i + " of " + lines.size());
            }
